...
/**
 * One-off: S-3 (docs/SALES-MODULE-PLAN.md) added SO to seed-number-
 ...
 * SO number_series row on their own, and nextNumber() throws "No number
 * series configured" the first time anyone tries to create a sales order.
 ...
 * the missing SO row gets inserted.
 */
...
main()
  .catch((error: unknown) => {
    logger.error({ err: error }, "sales number series backfill crashed");
    process.exitCode = 1;
  })
  .finally(async () => {
