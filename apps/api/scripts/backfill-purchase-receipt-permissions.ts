...
/**
 * One-off: PL-1 added purchase.receipt.create/confirm to manifests.ts,
 * same gap backfill-purchase-invoice-permissions.ts closed for
 * purchase.invoice.* - seedPermissionCatalogue only runs at
 * (re)provisioning time, so an already-active tenant never picks up a
 * manifest addition on its own. This upserts the catalogue for every
 * active tenant, then grants the 2 new keys to whichever of its default
 * roles already carry the matching tier (same ROLE_PERMISSION_FILTERS as
 * seed-roles.ts, now updated to include "confirm" at the Manager tier
 * alongside "approve"), skipping any role+permission pair already
 * granted.
 */
const NEW_KEYS = ["purchase.receipt.create", "purchase.receipt.confirm"] as const;
const ROLE_ACTION_FILTERS: Record<string, (action: string) => boolean> = {
  ...
  Manager: (action) => ["read", "create", "update", "approve", "confirm", "assign", "provision"].includes(action),
  Admin: () => true,
};
