...
/**
 * One-off: S-1 (docs/SALES-MODULE-PLAN.md) added CUSTOMER to
 * seed-number-series.ts's DEFAULT_SERIES, same gap
 * backfill-purchase-receipt-number-series.ts closed for PURCHASE_RECEIPT -
 * that list is only walked at (re)provisioning time, so an already-active
 * tenant's companies never get a CUSTOMER number_series row on their own,
 * and nextNumber() throws "No number series configured" the first time
 * anyone tries to create a customer via the new dedicated module.
 * seedDefaultNumberSeries is itself idempotent (onConflictDoNothing per
 * company/branch/docType/FY), so re-running it here for every company of
 * every active tenant is safe - existing series rows are untouched, only
 * the missing CUSTOMER row gets inserted.
 */
...
main()
  .catch((error: unknown) => {
    logger.error({ err: error }, "customer number series backfill crashed");
    process.exitCode = 1;
  })
  .finally(async () => {
