...
/**
 * One-off: C-3b (docs/CONTRACT-MODULE-BUILD.md) added CONTRACT to seed-
 ...
 * CONTRACT number_series row on their own, and nextNumber() throws "No
 * number series configured" the first time anyone tries to create a
 * contract. seedDefaultNumberSeries is idempotent (onConflictDoNothing per
 * company/branch/docType/FY), so re-running it for every company of every
 * active tenant only inserts the missing CONTRACT row.
 */
