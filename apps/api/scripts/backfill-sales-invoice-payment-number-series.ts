...
/**
 * One-off: S-5 (docs/SALES-MODULE-PLAN.md) added INVOICE and RECEIPT to
 * seed-number-series.ts's DEFAULT_SERIES - that list is only walked at
 * (re)provisioning time, so an already-active tenant's companies never get
 * an INVOICE/RECEIPT number_series row on their own, and nextNumber()
 * throws "No number series configured" the first time anyone tries to
 * create a sales invoice or record a payment received.
 ...
 * the missing INVOICE/RECEIPT rows get inserted. Mirrors backfill-sales-
 * delivery-number-series.ts exactly.
 */
...
main()
  .catch((error: unknown) => {
    logger.error({ err: error }, "sales invoice/payment number series backfill crashed");
    process.exitCode = 1;
  })
  .finally(async () => {
