...
/**
 * One-off: PL-5 added purchase.payment.record to manifests.ts - same gap
 * every prior new-permission backfill in this directory closes.
 * seedPermissionCatalogue only runs at (re)provisioning time, so an
 * already-active tenant never picks up a manifest addition on its own,
 * which is what produces "Unknown permission \"purchase.payment.record\""
 * from roles.service.ts's grantPermission/revokePermission the moment
 * anything (the frontend's mock catalogue, an admin granting the
 * permission to a role) tries to reference the new key against an old
 * tenant.
 *
 * This upserts the catalogue for every active tenant, then grants the new
 * key to whichever of a tenant's default SYSTEM roles already carry the
 * Manager tier (action "record" - deliberately not "create", see
 * manifests.ts's own doc comment: money leaving the company needs the
 * same bar as issue/approve/confirm, not the Officer-tier day-to-day
 * "create" bucket). Custom, non-default roles are left untouched - same
 * policy every prior backfill of this shape already established.
 */
const NEW_KEYS = ["purchase.payment.record"] as const;
