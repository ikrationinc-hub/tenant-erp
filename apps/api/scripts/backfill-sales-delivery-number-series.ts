...
/**
 * One-off: S-4 (docs/SALES-MODULE-PLAN.md) added DELIVERY to seed-number-
 * series.ts's DEFAULT_SERIES - that list is only walked at (re)
 * provisioning time, so an already-active tenant's companies never get a
 * DELIVERY number_series row on their own, and nextNumber() throws "No
 * number series configured" the first time anyone tries to create a
 * delivery. seedDefaultNumberSeries is itself idempotent (onConflictDoNothing
 * per company/branch/docType/FY), so re-running it here for every company
 * of every active tenant is safe - existing series rows are untouched,
 * only the missing DELIVERY row gets inserted. Mirrors backfill-sales-
 * number-series.ts exactly.
 */
...
main()
  .catch((error: unknown) => {
    logger.error({ err: error }, "sales delivery number series backfill crashed");
    process.exitCode = 1;
  })
  .finally(async () => {
