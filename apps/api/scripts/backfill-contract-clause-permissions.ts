...
/**
 * One-off: C-1 (docs/CONTRACT-MODULE-BUILD.md) added the "contract" module
 * with permissions contract.clause.read/create/version/approve/deactivate -
 * same gap every prior new-module/new-permission backfill in this directory
 * closes (seedPermissionCatalogue only runs at (re)provisioning time, so an
 * already-active tenant never picks up a manifest addition on its own).
 *
 * Mirrors backfill-purchase-payment-permission.ts's shape exactly, with one
 * addition: "version" is graded into the Officer tier here (see seed-
 * roles.ts's own updated doc comment - adding a clause version is routine
 * legal-content editing, not a governance action), which the ROLE_ACTION_
 * FILTERS below must match or Officer roles on already-active tenants would
 * silently never receive contract.clause.version.
 */
const NEW_KEYS = [
