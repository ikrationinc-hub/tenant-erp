...
function asHedge(res: { body: unknown }) {
  return hedgeSchema.parse(res.body);
}
...
async function seedTenant(label: string): Promise<SeededTenant> {
  ...
  const { companyId, userId, purchaseRefs, hedgePlatformId } = await withTenantSchema(tenant.schemaName, async (tx) => {
...
describe("modules/purchase - Platform Hedging / LME Records, session (d): hedging (docs/spec/Purchase-V2.md Sub Tab 3, B)", () => {
  afterAll(async () => {
  ...
  it(
    ...
    async () => {
      const tenant = await seedTenant("fr204");
      const app = createApp();
  ...
  it(
    "many hedges can be recorded against the same purchase (resolved open question #8)",
    async () => {
      const tenant = await seedTenant("many-hedges");
      const app = createApp();
      const authHeader = `Bearer ${tenant.accessToken}`;
      const purchaseId = await createDraftPurchase(app, authHeader, tenant);
      ...
      await request(app)
        .post(`/api/v1/purchases/${purchaseId}/hedges`)
        .set("Authorization", authHeader)
        .send({ hedgePlatformId: tenant.hedgePlatformId, contractNumber: "HC-A", position: "buy", quantity: "300", rate: "8500", hedgeDate: "2024-06-11" });
      await request(app)
        .post(`/api/v1/purchases/${purchaseId}/hedges`)
        .set("Authorization", authHeader)
        .send({ hedgePlatformId: tenant.hedgePlatformId, contractNumber: "HC-B", position: "sell", quantity: "200", rate: "8550", hedgeDate: "2024-06-12" });

      const getRes = await request(app).get(`/api/v1/purchases/${purchaseId}`).set("Authorization", authHeader);
      const hedgeList = (getRes.body as { hedges: unknown[] }).hedges;
      expect(hedgeList).toHaveLength(2);
    },
    TEST_TIMEOUT_MS,
  );
  ...
  it(
    ...
    async () => {
      ...
      const res = await request(app)
        .post(`/api/v1/purchases/${purchaseId}/hedges`)
        .set("Authorization", authHeader)
        .send({ hedgePlatformId: tenant.hedgePlatformId, contractNumber: "HC-late", position: "buy", quantity: "100", rate: "8600", hedgeDate: "2024-07-01" });
      expect(res.status).toBe(201);
    },
    TEST_TIMEOUT_MS,
  );
  ...
  it(
    "a hedge's status can move open -> closed while its contract terms stay immutable",
    async () => {
      const tenant = await seedTenant("close-hedge");
      const app = createApp();
      const authHeader = `Bearer ${tenant.accessToken}`;
      const purchaseId = await createDraftPurchase(app, authHeader, tenant);
      ...
      const hedge = asHedge(
        await request(app)
          .post(`/api/v1/purchases/${purchaseId}/hedges`)
          .set("Authorization", authHeader)
          .send({ hedgePlatformId: tenant.hedgePlatformId, contractNumber: "HC-2002", position: "sell", quantity: "400", rate: "8700", hedgeDate: "2024-06-13" }),
      );

      const closeRes = await request(app)
        .patch(`/api/v1/purchases/${purchaseId}/hedges/${hedge.id}`)
        .set("Authorization", authHeader)
        .send({ status: "closed" });
      expect(closeRes.status).toBe(200);
      const closed = asHedge(closeRes);
      expect(closed.status).toBe("closed");
      // Contract terms unchanged.
      expect(closed.contractNumber).toBe(hedge.contractNumber);
      expect(closed.rate).toBe(hedge.rate);
      expect(closed.quantity).toBe(hedge.quantity);
    },
    TEST_TIMEOUT_MS,
  );
});
