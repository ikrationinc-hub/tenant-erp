...
export const FIELD_DEFAULTS: FieldDefault[] = [
  ...
  // PL-2 added dueDate/taxAmount to purchase_bills but never their form
  // fields (backend-only prompt) - PL-4 closes that gap. taxAmount is a
  // clean seam only (CLAUDE.md's "Do NOT import from Zoho" - no TDS/TCS
  // mechanics), reference-only, never computed or enforced.
  {
    module: "purchase",
    entity: "invoice",
    fieldKey: "dueDate",
  ...
  // PL-4: the Purchase Receipt's own header fields (receiptDate/
  // warehouseId) - receiptNumber is server-generated (AutoGenerated, same
  // treatment as purchaseNumber/invoiceNumber). The repeating item/
  // quantity lines are hand-built UI (PurchaseDetailScreen.tsx's
  // PurchaseReceiptForm), not field-definitions fields - a per-item
  // outstanding-qty-capped grid isn't a form shape the field engine
  // describes, same category as the read-only Items table already is.
  {
    module: "purchase",
    entity: "receipt",
  ...
  // PL-5: Payment's own header fields - paymentNumber is server-generated
  // (AutoGenerated, same treatment as purchaseNumber/receiptNumber/
  // invoiceNumber). The repeating "which bill(s), how much applied to
  // each" allocation grid is hand-built UI (PurchasePaymentForm), not
  // field-definitions fields - same category as the Receipt/Bill forms'
  // own outstanding-qty-capped grids, which aren't a shape the field
  // engine describes either.
  {
    module: "purchase",
    entity: "payment",
  ...
  },
  {
    ...
    isSystem: false,
    sortOrder: 3,
    // A fixed, structural set of values (the paymentModeEnum column
    // itself), not tenant-configurable data - same reasoning as
    // pricingType's own staticOptions treatment above.
    optionsSource: {
      type: "enum",
      staticOptions: [
        { label: "Cash", value: "cash" },
